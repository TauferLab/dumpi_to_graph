...
//  for ( auto kvp : this->chan_to_send_seq ) {
...
//      auto recv_seq = this->chan_to_recv_seq.at( kvp.first );
//#if SANITY_CHECK
...
//      this->chan_to_send_seq.erase( kvp.first );
//      this->chan_to_recv_seq.erase( kvp.first );
//    }
//  }
}
...
//  mpi_rc = MPI_Comm_rank(MPI_COMM_WORLD, &rank);
//
//  int n_recv_reqs = this->chan_to_send_seq.size();
...
//  std::unordered_map<Channel, int, ChannelHash> chan_to_offset;
//  int n_recv_vertex_ids = 0;
//  for ( auto kvp : this->chan_to_send_seq ) {
//    chan_to_offset.insert( { kvp.first, n_recv_vertex_ids } );
...
//    int recv_buffer_offset = chan_to_offset.at( kvp.first );
...
//  for ( auto kvp : this->chan_to_recv_seq ) {
...
void EventGraph::make_collective_message_edges()
{
